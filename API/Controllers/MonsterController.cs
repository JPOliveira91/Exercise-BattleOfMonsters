...
public class MonsterController : BaseApiController
{
    ...
    public async Task<ActionResult> Find(int id)
    {
        ...
        if (monster == null) return new NotFoundObjectResult($"The monster with ID = {id} not found.");
    ...
    public async Task<ActionResult> Update(int id, [FromBody] Monster monster)
    {
        ...
        if (existingMonster == null) return new NotFoundObjectResult(string.Format($"The monster with ID = {id} not found."));

        _repository.Monsters.Update(existingMonster, monster);
    ...
    {
        var entity = await _repository.Monsters.FindAsync(id);

        if (entity == null) return new NotFoundObjectResult(string.Format($"The monster with ID = {id} not found."));

        _repository.Monsters.RemoveAsync(entity);
        await _repository.Save();
        return Ok();
    }
